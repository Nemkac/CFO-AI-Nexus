...
const Numbers = ({ cmsData }: { cmsData?: PromoData['numbers'] }) => {
    const { openModal } = useRegistration()
    const cards = cmsData?.cards?.length ? cmsData.cards : FALLBACK_CARDS
    const title = cmsData?.title ?? 'The AI hype is loud. The actual roadmap is missing.'
    const subtitle = cmsData?.subtitle ?? 'Every CFO knows AI is coming. Few know what to do on Monday morning. Vendors promise the world. The board wants answers. Your team is waiting for direction. And the cost of getting it wrong keeps climbing.'
    const ctaDescription = 'Joined by 10,000+ finance leaders. Seats are limited.'
    const ctaButtonLabel = 'Reserve Your Seat'
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full max-w-6xl">
                ...
                >
                    <div className='flex flex-col w-full items-center justify-center h-full gap-6'>
                        <Button variant="sky" label={ctaButtonLabel} onClick={openModal} />
                        <p className="text-p-md text-content-heading text-balance">{ctaDescription}</p>
                    </div>
                </motion.div>
            </div>
        </section>
